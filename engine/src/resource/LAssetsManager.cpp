...
#include "../../include/resource/LAssetsManager.h"
...
namespace engine
{
    ...
    LAssetsManager::LAssetsManager()
    {
        ...
        _cubeTextureData = loadCubeTexture( "orbital-element" );
    ...
    LTextureData* LAssetsManager::loadTexture( const char* filename, const char* ftype )
    {
        ...
        string _strFilename = ENGINE_IMGS_PATH + string( filename ) + ftype;
    ...
    LCubeTextureData* LAssetsManager::loadCubeTexture( const char* basename, const char* ftype )
    {
        ...
        for ( int q = 0; q < 6; q++ )
        {
            string _resPath = basename + string( "_" ) + s_cubeVariations[q];
